// bh_ascii.c — Simplified Schwarzschild ASCII rings (depth-biased ramp)
// build: cc -O3 -lm bh_ascii.c -o bh_ascii
// run:   ./bh_ascii [inc_deg] [FOVx_deg] [robs]
...
static inline double ring_mul(double r){
  ...
  // Periodic bands; tweak constants for visual taste
  const double N = 12.0;            // number of bands across [rin,rout]
  const double sharp = 1.6;         // sharpen bright bands
  const double floor = 0.25;        // darkness of gaps
  const double peak  = 1.20;        // brightness of bands
  double w = 0.5*(1.0 + cos(2.0*M_PI*N*s)); // 1 in band centers, 0 in gaps
  double t = pow(w, sharp);
  return floor + (peak - floor) * t;
}

// Minimal azimuthal modulation for animation (rotates with phase)
static inline double az_mul(double phi, double phase){
  const double amp = 0.25; // small, to avoid overstyling
  double m = 1.0 + amp * cos(phi + phase);
  if(m < 0.10) m = 0.10;   // clamp to avoid full dark
  return m;
}

typedef struct { double r, phi, g, emiss, depth; int hit; } Hit; // hit=1 if disk
...
static Hit trace_pixel(int px,int py){
  ...
  double lam = 0.0, lam_prev = 0.0; // affine step accumulator for depth proxy
  for(int step=0; step<5000; ++step){
    ...
    if((th_prev-M_PI/2.0)*(x[2]-M_PI/2.0)<=0.0){
      ...
      if(rhit>=rin && rhit<=rout){
        ...
        H.depth = lam_prev + f*(lam - lam_prev);
        return H;
      }
    }
...
static char RAMP[] = " .,:;-~=*%#@";
static double gamma_c = 0.45; // display gamma

int main(int argc, char** argv){
  // Observer-only CLI controls: inc_deg, FOVx (deg), robs
  if(argc > 1){ double val = atof(argv[1]); if(val > 0.0 && val < 89.0) inc_deg = val; }
  ...
  for(;;){
    ...
    for(int y=0;y<HEIGHT;y++) for(int x=0;x<W;x++){
      double val=0.0;
      if(map[y][x].hit){
        double g3 = pow(map[y][x].g,3.0);                 // classic Doppler/grav beaming
        double base = map[y][x].emiss * g3 * ring_mul(map[y][x].r);
        // apply azimuthal modulation that rotates with phase
        val = base * az_mul(map[y][x].phi, phase);
        if(map[y][x].depth < dmin) dmin = map[y][x].depth;
        if(map[y][x].depth > dmax) dmax = map[y][x].depth;
      }
      I[y][x]=val; if(val>Imax) Imax=val;
    }
    // draw frame with depth-biased ASCII ramp
    double inv_depth_span = (dmax>dmin)? 1.0/(dmax-dmin): 0.0;
    int ramp_len = (int)sizeof(RAMP) - 1; // exclude null
    const int depth_push = 6;             // extra brighter steps for near pixels
    for(int y=0;y<HEIGHT;y++){
      for(int x=0;x<W;x++){
        double v = (Imax>1e-12)? (I[y][x]/Imax) : 0.0;
        ...
        int idx = idx_base;
        if(map[y][x].hit && inv_depth_span>0.0){
          double df = 1.0 - (map[y][x].depth - dmin) * inv_depth_span; // 1 near, 0 far
          int offset = (int)round(df * depth_push);
          idx += offset;
