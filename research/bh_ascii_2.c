...
const double hs_gain  = 1.2;  // slightly reduced to avoid overpowering
// Cloudiness parameters: multi-harmonic modulation moving with phase
const double cloud_gain = 0.6;   // overall strength
const double cloud_k1   = 1.0;   // fundamental weight
const double cloud_k2   = 0.7;   // 2nd harmonic weight
const double cloud_k3   = 0.5;   // 3rd harmonic weight
const double cloud_s1   = 0.25;  // radial falloff for k1
const double cloud_s2   = 0.30;  // radial falloff for k2
const double cloud_s3   = 0.35;  // radial falloff for k3
...
int main(int argc, char** argv){
  ...
  for(int k=0;k<frames;k++){
    ...
    for(int y=0;y<HEIGHT;y++) for(int x=0;x<W;x++){
      double val=0.0;
      if(map[y][x].hit){
        double base = map[y][x].emiss * pow(map[y][x].g,3.0);
        double dphi = atan2(sin(map[y][x].phi - phase), cos(map[y][x].phi - phase));
        double hs_boost = hs_gain*exp(-(dphi*dphi)/(2*hs_sigma*hs_sigma));
        double r = map[y][x].r;
        // cloudiness: moving waves; additive positive-only so base never dims
        double phi = map[y][x].phi;
        double c1 = cloud_k1 * cos(phi - phase) * exp(-(r-rin)*cloud_s1/(rout-rin));
        double c2 = cloud_k2 * cos(2*(phi - 1.7*phase)) * exp(-(r-rin)*cloud_s2/(rout-rin));
        double c3 = cloud_k3 * sin(3*(phi - 2.3*phase)) * exp(-(r-rin)*cloud_s3/(rout-rin));
        double csum = c1 + c2 + c3;
        double cloud_pos = csum > 0.0 ? csum : 0.0;
        val = base * (1.0 + hs_boost + cloud_gain * cloud_pos);
      }
